...
void main() {
  ...
  test('this repo vends NO operator assets of its own — extension/ is gone', () {
    expect(
      Directory(p.join(repoRoot, 'extension')).existsSync(),
      isFalse,
      reason:
  ...
  test('nothing under .claude/ is a SYMLINK — the installed assets are real, '
    ...
    final links = Directory(p.join(repoRoot, '.claude'))
    ...
    expect(
      links,
      isEmpty,
      reason: 'symlinked assets:\n  ${links.join('\n  ')}',
    );
  });

  test('every installed operator asset is present and PROVENANCE-STAMPED', () {
    for (final relative in installed) {
      final file = File(p.join(repoRoot, relative));
      expect(
        file.existsSync(),
        isTrue,
        reason:
            '$relative is missing — run `dart run space:space assets install`',
      );
      expect(
