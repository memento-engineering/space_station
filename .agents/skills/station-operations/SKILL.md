...
compatibility: Requires dart, the `dart run space:space` verb runner, and `dart run space:space` to start the resident (which runs JIT via `dart run`, never an AOT binary), bd (beads CLI), git.
metadata:
...
A governor session itself is started with `dart run space:space seat governor`, not a bare
